...
function [ psiLR, e0 ] = eigs_Schwinger_superblock_guess_rtol_ss(HLss, QLss, ...
                                     SpLss, IdLss, UL, TL, ...
                                     HRss, SRss, SpRss, IdRss, UR, psi, ...
...
% Step 1: Contraction of psi0 with UR^*
for j0 = 1:k0
    psi0s1{j0} = psi0{j0}*conj(UR{j0});
end
...
for j0 = 1:k0
    p = 1;
    slj = (N(1) + 2)/2 + 1 + sct0(j0, 1);    % s_{l,j}
    kij = size(TL{slj}, 1);                  % k_{ij}
    for ij = 1:kij
        m1 = TL{slj}(ij, 3);              % m^{s_{l-1,ij}}
        m2 = TL{slj}(ij, 4);              % m^{sigma_{l, ij}}
        m3 = size(psi0s1{j0}, 2);         % m^{lambda_{r, j}}
        psi0s2a{j0}{ij} = permute(reshape(psi0s1{j0}(p:p + m1*m2 - 1, :), [m2, m1, m3]), [2, 1, 3]);
        p = p + m1*m2;
    end
end
...
for j = 1:k
    ...
    for j0 = 1:k0
        ...
        for ij = 1:kij
            m1 = TL{slj}(ij, 3);              % m^{s_{l-1,ij}}
            m2 = TL{slj}(ij, 4);              % m^{sigma_{l, ij}}
            m3 = size(psi0s2a{j0}{ij}, 3);    % m^{lambda_{r, j}}
            sgj = TL{slj}(ij, 2);             % sigma_{l, ij}
            if sct0(j0, 2) + sgj == sct(j, 2)
                psi0s2b{j}(:, p:p + m2*m3 - 1) = reshape( permute(psi0s2a{j0}{ij}, [1, 3, 2]),[m1, m2*m3]);
                p = p + m2*m3;
            end
        end
    end
...
% Step 3: Contraction psi0s2b with UL
% adjust UL for the third quarter of the first sweep (some UL{j} may not exists before):
if sweep == 1 && (N(1) < N(2))
    sct_old = spin_combinations([N(1),  N(1)], s);
    if isempty(sct_old)
        for j = 1:k
            UL_new{j} = eye(dsl(j), size(psi0s2b{j}, 1));
        end
    else
        for j = 1:k
            j_old = find(sct_old(:, 1) == sct(j, 1));
            if isempty(j_old)
                UL_new{j} = zeros(dsl(j), size(psi0s2b{j}, 1));
            else
                UL_new{j} = UL{j_old};    
            end
        end
