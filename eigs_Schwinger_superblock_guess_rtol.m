...
% the superblock for the XY model in a given total spin projection s
% and uses guessing algorithm for Right to Left step of the sweep
% and uses Dempsey et all convention
% Input: 
%        HL, QL, SpL:   left blocks
%        mL: dimensions of left blocks 
%        HR, SR, SpR:  right blocks
%        mR: dimensions of right blocks 
%        UL(NL) and UR(NR - 1): unitary matrices for NL and NR - 1
%        TL(NL + 1), TR(NR): spins and dimensions ledgers 
%        mL: dimensions of left blocks 
%        mR: dimensions of right blocks 
...
function [ psiLR, e0, idx_qc ] =  ...
               eigs_Schwinger_superblock_guess_rtol(HL, QL, SpL, mL, ...
                               HR, SR, SpR, mR, UL, TL, UR, TR, ...
                               psi0, idx_qc0, idx_qc_old, mLb, ...
                               N, s, x, y, theta, lvs, kryl, eig_tol, maxit)
...
if mSBs > 1
    ...
    % check that number of eigenvalues is not larger than the superblock size ds
    if lvs < mSBs
        levels = lvs;
    end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% Eigenstate guessing part: from right to left  %%%%%%%%%%%%%%%%%%%%
% Get the number of the previous quantum combinations j0 = 1,...,k0 
k0 = size(idx_qc0, 1);  

% Step 1: Contraction of psi0 with UR^*(NR - 1)
psi0p = cell(1, k0);
for j0 = 1:k0
    psi0p{j0} = psi0{j0}*conj(UR{j0});
end
...
for j = 1:size(idx_qc, 1)
    ...
    q_lm1j = idx_qc(j, 3);
    p = 1;
    for ij = 1:size(TR{i}, 1)        
        q_rij = TR{i}(ij, 2);
        mRb_rij = TR{i}(ij, 4);
        mS_ij = TR{i}(ij, 3);
        j0 = find(idx_qc0(:, 4) == q_rij);
        q_lj0 = idx_qc0(j0, 3);
        i0 = idx_qc0(j0, 1);
        ij0 = find(TL{i0}(:, 1) == q_lm1j);
        if ~isempty(ij0)
            mLb_lm1j = TL{i0}(ij0, 3);
            p0 = 1 + TL{i0}(1:(ij0-1), 3)'*TL{i0}(1:(ij0-1), 4);
            psi0p2{j}(:, p:p + mS_ij*mRb_rij - 1) = ...
                  permute(reshape( permute(...
                    psi0p{j0}(p0:p0 + mS_ij*mLb_lm1j - 1, :), [2, 1]), ...
                    [mS_ij*mRb_rij, mLb_lm1j] ), [2, 1]);
            p = p + mS_ij*mRb_rij;
        else
            psi0p2{j}(:, p:p + mS_ij*mRb_rij - 1) = zeros(0, mS_ij*mRb_rij);
            p = p + mS_ij*mRb_rij;
        end
    end
end

% Step 3: Contraction psi0p2 with UL
...
if isempty(idx_qc_old)
    ...
    for j = 1:k
        ...
        if ~isempty(psi0p2{j})
            % Multiply by UL
            psi0p3{j} =  speye(mLc(j), size(psi0p2{j}, 1))*psi0p2{j};
        else
    ...
    for j = 1:k
        if ~isempty(psi0p2{j})
            ...
            j_old = find(idx_qc_old(:, 3) == idx_qc(j, 3));
            if isempty(j_old)
                %disp('no j_old')
                psi0p3{j} =  speye(mLc(j), size(psi0p2{j}, 1))*psi0p2{j};
            else
                % Multiply by UR^T.
                psi0p3{j} =  UL{j_old}*psi0p2{j};
            end
        else
            psi0p3{j} =  zeros(mLc(j), mRc(j));
        end
...
% and it uses Dempsey et all convention
% Input: 
